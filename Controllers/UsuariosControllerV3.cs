...
{
	public class UsuariosControllerv3 : Controller
	{
		private readonly IConfiguration configuration;
		private readonly IWebHostEnvironment environment;
		private readonly IRepositorioUsuario repositorio;

		public UsuariosControllerv3(IConfiguration configuration, IWebHostEnvironment environment, IRepositorioUsuario repositorio)
		{
		...
		{
			var e = repositorio.ObtenerPorId(Id_Usuario);
			return View(e);
		}
		...
		public ActionResult Create(Usuario u)
		{
			...
			try
			{
				...
				if (u.AvatarFile != null && u.Id_Usuario > 0)
				{
					...
					string fileName = "avatar_" + u.Id_Usuario + Path.GetExtension(u.AvatarFile.FileName);
		...
		public ActionResult Edit(int Id_Usuario)
		{
			...
			var u = repositorio.ObtenerPorId(Id_Usuario);
			ViewBag.Roles = Usuario.ObtenerRoles();
			return View(u);
		}
		...
		[ValidateAntiForgeryToken]
		[Authorize]
		public ActionResult Edit(int Id_Usuario, Usuario u)
		{
			...
			try
			{
				if (!User.IsInRole("Administrador"))//no soy admin
				{
					...
					if (usuarioActual.Id_Usuario != Id_Usuario)//si no es admin, solo puede modificarse él mismo
		...
		[ValidateAntiForgeryToken]
		[Authorize(Policy = "Administrador")]
		public ActionResult Delete(int Id_Usuario, Usuario usuario)
		{
			try
			{
				// TODO: Add delete logic here
				var ruta = Path.Combine(environment.WebRootPath, "Uploads", $"avatar_{Id_Usuario}" + Path.GetExtension(usuario.Avatar));
		...
		{
			var u = repositorio.ObtenerPorEmail(User.Identity.Name);
			string fileName = "avatar_" + u.Id_Usuario + Path.GetExtension(u.Avatar);
			string wwwPath = environment.WebRootPath;
			string path = Path.Combine(wwwPath, "Uploads");
			string pathCompleto = Path.Combine(path, fileName);
		...
		public ActionResult Datos()
		{
			...
			{
				var u = repositorio.ObtenerPorEmail(User.Identity.Name);
				string buffer = "Nombre;Apellido;Mail" + Environment.NewLine +
		...
		public async Task<IActionResult> Login(Login login)
		{
			try
			{
				...
				if (ModelState.IsValid)
				{
					...
					{
						ModelState.AddModelError("", "El Mail o la Password no son correctos");
						TempData["returnUrl"] = returnUrl;
						return View();
					}
